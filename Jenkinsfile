    ...
    stage('Checkout from Git') {
      ...
      steps {
        sh '''
          ...
          echo "${ssh_private_key}" > /var/lib/jenkins/ssh-certs/id_rsa.pem
          echo "${ssh_public_key}" > /var/lib/jenkins/ssh-certs/id_rsa.pem.pub
        '''
      }
    }
    stage('Adding Public Certificate to Remote Host ') {
      steps {
        sh 'ssh-copy-id -i /var/lib/jenkins/ssh-certs/id_rsa.pem.pub ${Username}@${Hostname}'
      }
    }
